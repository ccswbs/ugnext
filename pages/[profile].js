...
export default function ProfilePage() {
  ...
  //const { state } = router;  Access the state passed via the Link component
  const { userId } = router.query ? router.query : null;
    ...
    return (
      <Layout metadata={{ title: "UniWEB Test" }}>
        <Container centered>
          ...
          <p>User ID is missing. Please provide a valid User ID in order to display profile info.</p>
        </Container>
      </Layout>
    );
  }
      ...
      <Container centered>
        ...
        <div className="flex flex-col md:flex-row mb-3">
          ...
          <div className="w-full md:flex-1 p-4">
            <span className="text-xl font-semibold">{membershipInfo.position_title[1]}</span>
            <br />
            {membershipInfo.academic_unit[1]}
            <br />
            {membershipInfo.academic_unit[2]}
            <br />
            {membershipInfo.academic_unit[3]}
            <br />
            {membershipInfo.email && (
              <>
                <i className="fa-solid fa-envelope"></i> {membershipInfo.email}
                <br />
              </>
            )}
            {membershipInfo.office && (
              <>
                <i className="fa-solid fa-building"></i> Office: {membershipInfo.office}
                <br />
              </>
            )}
            {membershipInfo.telephone && (
              <>
                <i className="fa-solid fa-phone"></i> {formatPhoneNumber(membershipInfo.telephone)}
                <br />
              </>
            )}
            {membershipInfo.homepage && (
              <>
                <i className="fa-solid fa-globe-pointer"></i> <Link href={membershipInfo.homepage}>Website</Link>
              </>
            )}
          </div>
        </div>
