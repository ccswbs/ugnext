...
export const nameAndTagSearch = (data) => {
  const parse = (input) => {
    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      for (let j = 0; j < node.tags.length; j++) {
        ...
        if (Array.isArray(tag)) {
          ...
          // If we found a match, we need to check the index of the match,
          // as we will need to check whether the previous parsedWord matched the previous subtag.
          if (match !== MATCH_TYPES.NONE) {
            if (index > 0) {
  ...
  const nodes = data
    .map((node) => ({
      data: node,
      keywords: parse(node.name || node.title),
      tags: node.tags?.map((tag) => {
        // Handle tag objects with name property
        const tagName = typeof tag === 'string' ? tag : tag?.name || '';
        return tagName.includes(" ") ? tagName.split(" ") : tagName;
      }).filter(tag => tag) ?? [],
    }))
