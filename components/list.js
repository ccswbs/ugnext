...
const List = ({ variant = 'unordered', children, className, ...rest }) => {
	...
	const itemClasses = twJoin(
		'h-fit w-full',
		variant !== 'ordered' &&
			'relative pl-8 before:absolute before:left-[1rem] before:top-[0.35rem] before:block before:h-[1.8rem] before:w-[1.1rem] before:shrink-0 before:items-center before:justify-center before:text-yellow before:content-[url("/icons/chevron-right.svg")]',
		variant === 'ordered' &&
