...
export function Video({ videoData, className }) {
  ...
  return (
    <>
      <div className={twJoin(embedResponsive, className, 'relative')} style={{ paddingTop: '56.25%' }}>
        <iframe
          className="w-full h-full absolute top-0 left-0"
          allowFullScreen
          src={videoSrc}
          title={videoTitle ? videoTitle : videoType + ' video player'}
        />
      </div>
      {videoTranscript && (
        <a
          className="flex justify-center items-center text-white bg-red-700 w-full py-2 btn btn-primary"
          href={videoTranscript}
        >
          Download transcript<span className="visually-hidden"> for {videoTitle + ' video'}</span>
        </a>
      )}
    </>
  );
