...
namespace ContinuingEducationPrograms {
...
export async function getContinuingEducationProgramType(filepath: string) {
  ...
  } as ContinuingEducationPrograms.ProgramType;
}
...
export async function getContinuingEducationProgram(filepath: string) {
  ...
  const programTypes: ContinuingEducationPrograms.ProgramType[] = [];

  for(const type of program.data.types) {
    try {
      programTypes.push(await getContinuingEducationProgramType(path.join(CONTINUING_EDUCATION_PROGRAM_TYPES_ROOT, `${type}.yml`)));
    } catch(e) {
  ...
  } as ContinuingEducationPrograms.Program;
}
